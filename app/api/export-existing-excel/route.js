...
// Normalize and flexible getter for nested structures
...
export async function GET(request) {
  try {
    ...
    if (parsedIds.size > 0) {
      ...
      console.log('[export-existing-excel] Snapshot size:', snap.size);
      if (snap.empty) {
        console.warn('[export-existing-excel] Tidak ada data dari Firestore (Valid_Survey_Data)');
      }
      snap.forEach((doc) => {
        const data = doc.data() || {};
        console.log('[export-existing-excel] doc:', doc.id, data);
        items.push({ id: doc.id, ...data });
      });
    }

    console.log('[export-existing-excel] Total items to export:', items.length);
    if (items.length === 0) {
      console.warn('[export-existing-excel] Peringatan: items kosong. Excel akan berisi header saja.');
    }

    // Workbook & worksheet
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('Rekap_Survey_Existing');

    // Header diselaraskan dengan format survey existing (lihat TODO_EXPORT_FORMAT_UPDATE.md)
    ws.columns = [
    ...
    items.forEach((d) => {
      ...
      // Set hyperlink foto dengan URL yang dapat diakses
      ...
      if (fotoArm) {
        let fotoArmUrl = fotoArm;
        // Convert relative path to absolute URL
        if (fotoArm.startsWith('/')) {
          fotoArmUrl = `${baseUrl}${fotoArm}`;
        }
        // If it's a Firebase Storage URL, use it directly
        else if (fotoArm.includes('firebasestorage.googleapis.com') || fotoArm.includes('storage.googleapis.com')) {
          fotoArmUrl = fotoArm;
        }
        // If it's already an absolute URL, use it directly
        else if (fotoArm.startsWith('http')) {
          fotoArmUrl = fotoArm;
        }
        ...
        row.getCell(colFotoArm).value = { 
          text: 'Lihat Foto ARM', 
          hyperlink: fotoArmUrl 
        };
        // Add blue color and underline to make it look like a proper link
        row.getCell(colFotoArm).font = { 
          color: { argb: 'FF0000FF' }, 
          underline: true 
        };
      }
    ...
    const fileName = 'Survey_Existing_Report.xlsx';
    ...
    console.error('export-existing-excel error:', err);
